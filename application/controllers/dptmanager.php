...
class Dptmanager extends MY_Controller
{
    ...
    public function custom_user_insert_stu_callback($post)
    {
        return $this->flexi_auth->insert_user($this->input->post('uacc_email'), $this->input->post('uacc_email'),
            $this->input->post('uacc_password'), array('department_id' => $this->user_data[0]->department_id, 'uacc_group_fk' => 3), 3, 1);
    }

    public function custom_user_insert_tea_callback($post)
    {
        return $this->flexi_auth->insert_user($this->input->post('uacc_email'), $this->input->post('uacc_email'),
            $this->input->post('uacc_password'), array('department_id' => $this->user_data[0]->department_id, 'uacc_group_fk' => 2), 2, 1);
    }

    public function assign_thesis()
    {
        // gia kathe thesis :

            // 1. bgazoume ektos tous foitites pou exoun idi kapoio assignment

            // 2. na paroume tous foitites pou exoun perasei ta proapaitoumena

            // 3. pairnoume ta kritiria
