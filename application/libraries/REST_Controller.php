...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('rest_enable_keys') and $use_key and ! empty($this->rest->key) and ! $this->_check_access())
        {
            ...
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => $controller_method . 'This API key does not have access to the requested controller.'), 401);
        }

        // Sure it exists, but can they do anything with it?
        if (!method_exists($this, $controller_method))
        {
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => $controller_method . ' : Unknown method.'), 404);
        }
